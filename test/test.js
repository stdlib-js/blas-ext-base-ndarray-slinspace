...
tape( 'the function fills a one-dimensional ndarray (endpoint=true)', function test( t ) {
	...
	x = ndarray( 'float32', new Float32Array( [ 0.0, 0.0, 0.0, 0.0 ] ), [ 4 ], [ 1 ], 0, 'row-major' );
	...
	actual = slinspace( [ x, start, stop, endpoint ] );
	t.strictEqual( actual, x, 'returns expected value' );
	t.strictEqual( isEqualDataType( getDType( actual ), 'float32' ), true, 'returns expected value' );
	t.deepEqual( getData( actual ), new Float32Array( [ 0.0, 1.0, 2.0, 3.0 ] ), 'returns expected value' );
...
tape( 'the function fills a one-dimensional ndarray (endpoint=false)', function test( t ) {
	...
	x = ndarray( 'float32', new Float32Array( [ 0.0, 0.0, 0.0 ] ), [ 3 ], [ 1 ], 0, 'row-major' );
	...
	t.strictEqual( actual, x, 'returns expected value' );
	t.strictEqual( isEqualDataType( getDType( actual ), 'float32' ), true, 'returns expected value' );
	t.deepEqual( getData( actual ), new Float32Array( [ 3.0, 2.0, 1.0 ] ), 'returns expected value' );
